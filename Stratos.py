...
for i in range(n_snapshots):  #--- Note that we are increasing the snapshot number by 10 (i*10). At the end we should do all 150 snapshots
    ...
    params_i = ["/dfsmount/miguel/MIP-2/RUNS/00-00-00/OUT/run_" + str(i*10).zfill(3),14452.3,28617.4,29517.4,150]
...
for w in range(len(profile)):
    ...
    for i in range(len(lines)-1):  #--- OJO fix -1, last element is empty!!!
        ...
        x.append(xyz[0]-14452.3+w*4000)  #--- damos un incremento en x para desplegar los snapshots de forma separada

        y.append(xyz[1]-28617.4)

        z.append(xyz[2]-29517.4)
